...
t_block_information* best_fit_available_partition_search_algorithm(uint32_t message_size,t_list* blocks_information, uint32_t min_partition_size){
    ...
    t_list* blocks_information_availables = list_filter(blocks_information, (bool (*)(void *)) _message_size_fits_partition);
    t_block_information* block_information_found = list_fold(blocks_information_availables, 0, (void* (*)(void *, void*)) _best_block_information);
