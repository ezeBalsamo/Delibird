...
void queue_context_add_subscriber_function(t_queue_context* queue_context, void* subscriber_socket_fd){
    pthread_mutex_lock(&queue_context -> subscribers_mutex);
    list_add(queue_context -> subscribers, subscriber_socket_fd);
...
t_queue_context_operations* new_queue_context_operations(){
    ...
    queue_context_operations -> push_message_status_to_queue_function = (void (*)(void *,
                                                                                  void *)) queue_context_push_message_status_to_queue_function;
