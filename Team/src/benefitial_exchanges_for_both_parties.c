...
t_list* completed_exchanges_for_first(t_trainer_thread_context* second_party_trainer_thread_context,
                                      t_list* benefitial_exchanges_for_first,
                                      t_list* benefitial_exchanges_for_second){

    t_list* completed_exchanges_for_first = list_duplicate(benefitial_exchanges_for_first);
    ...
    if(benefitial_exchanges_for_first_quantity < benefitial_exchanges_for_second_quantity){
        ...
        for(int i = 0; i < one_sided_benefitial_exchanges_amount; i++){
            ...
            list_add(completed_exchanges_for_first, pokemon_to_exchange);
        }

        list_destroy(remaining_pokemons_in_excess);
    }

    return completed_exchanges_for_first;
}

t_list* completed_exchanges_for_second(t_trainer_thread_context* first_party_trainer_thread_context,
                                       t_list* benefitial_exchanges_for_first,
                                       t_list* benefitial_exchanges_for_second){

    t_list* completed_exchanges_for_second = list_duplicate(benefitial_exchanges_for_second);

    int benefitial_exchanges_for_first_quantity = list_size(benefitial_exchanges_for_first);
    int benefitial_exchanges_for_second_quantity = list_size(benefitial_exchanges_for_second);

    if(benefitial_exchanges_for_second_quantity < benefitial_exchanges_for_first_quantity){

        t_waiting_for_exchange_action* waiting_for_exchange_action = internal_thread_action_in(first_party_trainer_thread_context);
        int one_sided_benefitial_exchanges_amount = benefitial_exchanges_for_first_quantity - benefitial_exchanges_for_second_quantity;
        ...
        for(int i = 0; i < one_sided_benefitial_exchanges_amount; i++){
            char* pokemon_to_exchange = list_get(remaining_pokemons_in_excess, i);
            list_add(completed_exchanges_for_second, pokemon_to_exchange);
        }

        list_destroy(remaining_pokemons_in_excess);
    }

    return completed_exchanges_for_second;
}
...
t_list* benefitial_for_both_parties_infer_function(t_trainer_thread_context* first_party_trainer_thread_context,
    ...
    t_list* exchanges_for_first =
            completed_exchanges_for_first(second_party_trainer_thread_context,
                                          benefitial_exchanges_for_first,
                                          benefitial_exchanges_for_second);

    t_list* exchanges_for_second =
            completed_exchanges_for_second(first_party_trainer_thread_context,
                                           benefitial_exchanges_for_first,
                                           benefitial_exchanges_for_second);
