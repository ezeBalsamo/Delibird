...
t_request* deserialize_appeared_pokemon(void* serialized_structure) {
    ...
    offset += sizeof(uint32_t);
    memcpy(&id_mensaje, serialized_structure + offset, sizeof(uint32_t));

    printf("request deserialized!\n");
    printf("request operation number: %d\n", APPEARED_POKEMON);
    printf("request structure: pokemon %s, %d posx, %d posy, %d id_mensaje.\n", pokemon_name, pos_x, pos_y,  id_mensaje);
}

t_request* deserialize_new_pokemon(void* serialized_structure){return NULL;}
t_request* deserialize_catch_pokemon(void* serialized_structure){return NULL;}
